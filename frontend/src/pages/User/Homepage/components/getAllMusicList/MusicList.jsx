...
const MusicList = () => {
  ...
  const { loading, error, musicsDetails } = useMusicListPage(page, 10, null);
  ...
  return (
    <section
      className={`py-10 lg:py-20 ${isDark ? "text-white" : "text-gray-900"}`}
    >
      {/* Header */}
      <div className="flex flex-col items-center text-center px-4 mb-10">
        ...
        <p
          className={`mt-3 text-base sm:text-lg max-w-xl ${isDark ? "text-gray-400" : "text-gray-500"}`}
        >
          Relax, focus, and meditate with our curated collection of soothing
          sounds
        </p>
      </div>
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        ...
        <div>
          ...
          <div className="flex flex-col items-start sm:items-end gap-3 mb-5">
            ...
            {totalRecords > 0 && (
              ...
              >
                {totalRecords} track{totalRecords !== 1 ? "s" : ""} • Page{" "}
                {currentPage}
              </p>
            )}
          </div>
        ...
        {loading && (
          <div className="animate-pulse flex gap-4 overflow-hidden">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className={`w-40 sm:w-48 flex-shrink-0 rounded-xl overflow-hidden ${isDark ? "bg-gray-800" : "bg-gray-100"}`}
              >
                <div
                  className={`h-40 sm:h-48 ${isDark ? "bg-gray-700" : "bg-gray-200"}`}
                />
                <div className="p-3 space-y-2">
                  ...
                  />
                  <div
                    className={`h-3 w-1/2 rounded ${isDark ? "bg-gray-700" : "bg-gray-200"}`}
                  />
                </div>
              </div>
            ))}
        ...
        {error && !loading && (
          <div
            className={`p-6 rounded-xl text-center border ${isDark ? "bg-red-900/20 border-red-800 text-red-300" : "bg-red-50 border-red-200 text-red-600"}`}
          >
        ...
        {!loading && !error && musics.length === 0 && (
          <div
            className={`py-16 rounded-2xl text-center border-2 border-dashed ${isDark ? "bg-gray-800/40 border-gray-700 text-gray-400" : "bg-gray-50 border-gray-200 text-gray-400"}`}
          >
              ...
              >
                {musics.map((music, index) => (
                  ...
                  >
                    {/* Circle thumbnail */}
                    <div
                      className={`relative w-32 h-32 sm:w-44 sm:h-44 rounded-full overflow-hidden border-4 transition-all duration-300 shadow-md ${
                        isDark
                    ...
                    >
                      ...
                      {/* Duration badge */}
                      <div
                        className={`absolute bottom-2 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 whitespace-nowrap ${isDark ? "bg-black/80 text-white" : "bg-white/95 text-gray-800"}`}
                      >
                      ...
                      {/* Play overlay */}
                      <div
                        className={`absolute inset-0 flex items-center justify-center bg-black/40 rounded-full transition-opacity duration-300 ${hoveredIndex === index ? "opacity-100" : "opacity-0"}`}
                      >
                        <div
                    ...
                    <div className="mt-3 text-center w-full px-1">
                      <h3
                        className={`text-xs sm:text-sm font-semibold line-clamp-1 ${isDark ? "text-white" : "text-gray-800"}`}
                      >
                        {music.music_title}
                      </h3>
                      <div className="flex items-center justify-center gap-1 mt-1">
                        <Volume2
                          className={`w-3 h-3 ${isDark ? "text-purple-400" : "text-purple-500"}`}
                        />
                        <span
                          className={`text-[10px] sm:text-xs truncate max-w-[90px] ${isDark ? "text-gray-400" : "text-gray-500"}`}
                        >
                          {domains.find((d) => d.domain_id === music.domain_id)
                            ?.domain_name ?? "Meditation"}
                        </span>
                      </div>
                    </div>
                  </motion.div>
          ...
          <div className="flex items-center justify-center gap-3 mt-8">
            {hasPrevPage && (
              <button
                ...
                disabled={loading}
                className={`px-5 py-2 rounded-full flex items-center gap-2 text-sm font-semibold transition-all disabled:opacity-50 ${
                  isDark
