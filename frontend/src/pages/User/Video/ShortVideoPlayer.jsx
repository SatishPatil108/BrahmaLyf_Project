...
const ShortVideoPlayer = () => {
  ...
  if (!video) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center transition-colors duration-300 ${colors.bg}`}
      >
        <div
          className={`p-6 rounded-2xl ${colors.cardBg} shadow-2xl text-center`}
        >
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${colors.bg}`}>
      ...
      <div className="max-w-lg mx-auto px-4 pt-20 sm:pt-24 pb-16 flex flex-col items-center">
        ...
        <h1
          className={`text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-6 sm:mb-8 ${colors.accentText}`}
        >
          {video.video_title.charAt(0).toUpperCase() +
            video.video_title.slice(1)}
        </h1>
        ...
        {video.video_description && (
          <div
            className={`w-full sm:w-[85%] md:w-[75%] mt-6 rounded-2xl ${colors.cardBg}  overflow-hidden`}
          >
            {/* Toggle header */}
            <button
            ...
            >
              <p
                className={`px-5 pb-5 text-sm sm:text-base leading-relaxed ${colors.mutedText}`}
                dangerouslySetInnerHTML={{
                  __html: video.video_description,
                }}
              />
            </div>
          </div>
