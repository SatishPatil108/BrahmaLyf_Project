...
const EnrolledCourses = () => {
  ...
  if (isLoading) {
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center p-4 lg:px-20  ${styles.bg}`}
      >
        <div className="text-center">
          <div className="relative mb-6">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <BookOpen
                className={`w-10 h-10 ${styles.emptyState.icon} animate-pulse`}
              />
            </div>
          </div>
          <p
            className={`text-lg font-medium ${styles.text.secondary} animate-pulse`}
          >
  ...
  if (error) {
    ...
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center p-6 lg:px-20 ${styles.bg}`}
      >
        <div
          className={`max-w-md w-full p-8 rounded-2xl ${styles.emptyState.bg} border ${theme === "dark" ? "border-gray-700" : "border-gray-200"} text-center`}
        >
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h3 className={`text-2xl font-bold mb-3 ${styles.text.primary}`}>
            Your Learning Journey Awaits
          </h3>
          <p className={`text-base mb-6 ${styles.text.secondary}`}>
            You haven't enrolled in any courses yet. Start your transformation
            journey today!
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
    ...
    <div className={`min-h-screen p-4 sm:p-6 lg:p-8 lg:px-20 ${styles.bg}`}>
              ...
              >
                ...
                <div className="h-40 overflow-hidden">
                  <div className="w-full h-full bg-gradient-to-br from-purple-500/20 to-pink-500/20 flex items-center justify-center">
                    <BookOpen
                      className={`w-16 h-16 ${styles.text.accent} opacity-50 group-hover:scale-110 transition-transform duration-300`}
                    />
                  </div>
                </div>
                ...
                <div className="p-5">
                  <h3
                    className={`text-lg font-bold mb-2 line-clamp-2 group-hover:text-purple-500 transition-colors ${styles.text.primary}`}
                  >
                    {course.course_name}
                  </h3>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-gray-700">
                    <span
                      className={`text-sm font-medium ${course.status === 1 ? "text-green-500" : "text-gray-500"}`}
                    >
                      {course.status === 1
                        ? "Continue Learning →"
                        : "View Course"}
                    ...
                    <ChevronRight
                      className={`w-5 h-5 ${styles.text.tertiary} group-hover:translate-x-1 transition-transform`}
                    />
                  </div>
                </div>
              </div>
      ...
      {myCourses.length === 0 && (
        <div className="text-center py-16">
          <BookOpen
            className={`w-16 h-16 mx-auto mb-4 ${styles.emptyState.icon}`}
          />
          <h3 className={`text-xl font-bold mb-2 ${styles.text.primary}`}>
            No courses enrolled
          </h3>
