...
import {
  FolderTree, Plus, SquarePen, Trash2,
...
const SubDomainsList = () => {
  ...
  // Display rows with JSX badge for difficulty — action handlers always use original `subdomains`
  const displayRows = subdomains.map((subdomain) => ({
    ...subdomain,
    progressive_difficulty: (
      <span
        className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
          ${COLORS.difficulty[subdomain.progressive_difficulty]?.bg ?? "bg-gray-100 dark:bg-gray-800"}
          ${COLORS.difficulty[subdomain.progressive_difficulty]?.text ?? "text-gray-700 dark:text-gray-300"}`}
      >
        {subdomain.progressive_difficulty} -{" "}
        {COLORS.difficulty[subdomain.progressive_difficulty]?.label ?? "Unknown"}
      </span>
    ),
  }));
  ...
  const openDrawerForEdit = (subdomain) => {
    ...
    setSubdomainThumbnail(null);
    setSubdomainThumbnailUrl(subdomain.subdomain_thumbnail ?? null);
    setErrors({});
    clearMessage();
    setIsDrawerOpen(true);
  };
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className={`mb-6 p-4 rounded-lg flex items-center gap-3 border ${
            ...
            <div className="flex-1">
              <p className={`font-medium ${
                actionMessage.type === "success" ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"
              }`}>
                {actionMessage.text}
              </p>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-lg font-medium text-gray-600 dark:text-gray-400">Loading subdomains...</p>
            <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">Please wait while we fetch categories</p>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">Failed to Load Subdomains</h2>
          ...
          <>
            <div className="overflow-x-auto bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 transition-colors duration-300">
              <CustomTable
                ...
                onEdit={(displayRow) => {
                  const original = subdomains.find((s) => s.subdomain_id === displayRow.subdomain_id);
                  if (original) openDrawerForEdit(original);
                }}
                onDelete={(displayRow) => {
                  const original = subdomains.find((s) => s.subdomain_id === displayRow.subdomain_id);
                  if (original) handleDeleteSubdomain(original);
                }}
              />
            </div>
            ...
            {subdomainsDetails.total_pages > 1 && (
              <div className="mt-6">
                <Pagination currentPage={pageNo} totalPages={subdomainsDetails.total_pages} onPageChange={setPageNo} />
              </div>
            )}
          </>
        )}
        ...
        {!loading && !error && subdomains.length === 0 && (
          <div className="text-center py-16 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-900">
            ...
            <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">No Subdomains Found</h3>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (errors.subdomainName) setErrors((p) => ({ ...p, subdomainName: null }));
                }}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (errors.difficultyLevel) setErrors((p) => ({ ...p, difficultyLevel: null }));
                }}
                className={`w-full px-4 py-3 rounded-lg border shadow-sm text-gray-900 dark:text-gray-100
                  bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 transition-all duration-200
            ...
            <div>
              ...
              <FileUploaderWithPreview
                ...
                setImageFile={(file) => {
                  ...
                  if (errors.subdomainThumbnail) setErrors((p) => ({ ...p, subdomainThumbnail: null }));
                }}
