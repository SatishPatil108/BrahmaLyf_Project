...
const useDailyShortList = (pageNo, pageSize) => {
  ...
  const { shortVideosDetails, loading, error } = useSelector(
    (state) => state.admin,
  );
  ...
  const addVideo = async (formData) => {
    ...
    try {
      ...
      setActionMessage({
        type: "success",
        text: "New Short Video is added successfully!",
      });
      refetch();
    } catch (err) {
  ...
  const updateShortVideoDetails = async (shortVideoId, shortVideoData) => {
    ...
    try {
      await dispatch(
        updateShortVideoAPI({ shortVideoId, shortVideoData }),
      ).unwrap();
      setActionMessage({
        type: "success",
        text: "Short Video is updated successfully!",
      });
      refetch();
    } catch (err) {
  ...
  const deleteShortVideo = async (shortVideoId) => {
    ...
    try {
      ...
      setActionMessage({
        type: "success",
        text: "Short Video is deleted successfully!",
      });
      refetch();
    } catch (err) {
