...
const ShortsList = () => {
  ...
  const domains = (domainsDetails?.domains ?? []).map(
    ({ domain_id, domain_name }) => ({ domain_id, domain_name }),
  );
  ...
  const handleEditShort = (short) => {
    ...
    setShortName(short.video_title);
    setShortInfo(short.video_description);
    setShortCategory(short.domain_id);
    setShortVideoUrl(short.video_file ?? "");
    setShortThumbnailUrl(short.video_thumbnail ?? null);
    setShortThumbnailFile(null);
    setErrors({});
    setTouched({});
    setIsDrawerOpen(true);
  ...
  const handleSubmit = async (ev) => {
    ...
    if (shortThumbnailFile && typeof shortThumbnailFile === "object")
      fd.append("video_thumbnail", shortThumbnailFile);
    try {
  ...
  const filteredVideos =
    activeDomainId === "all"
      ? videos
      : videos.filter((v) => v.domain_id === activeDomainId);
  ...
  const formatViews = (arr) => {
    const total = arr.reduce((s, v) => {
      ...
      return (
        s + parseFloat(raw.replace("K", "")) * (raw.includes("K") ? 1000 : 1)
      );
    }, 0);
      ...
      <div className="max-w-7xl mx-auto ">
          ...
          >
            {[{ domain_id: "all", domain_name: "All" }, ...domains].map((d) => {
              ...
              const count =
                d.domain_id === "all"
                  ? videos.length
                  : videos.filter((v) => v.domain_id === d.domain_id).length;
              return (
                <button
                  key={d.domain_id}
                  onClick={() => {
                    setActiveDomainId(d.domain_id);
                ...
                >
                  ...
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded-full ${
                      active ? "bg-indigo-500" : "bg-gray-200 dark:bg-gray-700"
                    }`}
                  >
                    {count}
                  </span>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-xl font-medium text-gray-600 dark:text-gray-400">
              Loading daily short video...
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">
              Please wait while we fetch your short video
            </p>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">
              Failed to Load daily short video
            </h2>
        ...
        {!loading && !error && filteredVideos.length === 0 && (
          <div className="text-center py-16 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-900">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6">
              Get started by adding your first short video
            </p>
            <CustomButton
              onClick={openAddDrawer}
              variant="primary"
        ...
        {!loading && (shortVideosDetails?.total_pages ?? 0) > 1 && (
          <div className="flex justify-center items-center gap-2 mt-8">
            {Array.from(
              { length: shortVideosDetails.total_pages },
              (_, i) => i + 1,
            ).map((p) => (
              <button
                key={p}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                value={shortCategory}
                onChange={(e) => {
                  setShortCategory(Number(e.target.value));
                  if (errors.shortCategory)
                    setErrors((p) => ({ ...p, shortCategory: null }));
                }}
            ...
            <div>
              <YouTubeUrlInput
                ...
                onChange={(e) => {
                  ...
                  if (errors.shortVideoUrl)
                    setErrors((p) => ({ ...p, shortVideoUrl: null }));
                }}
            ...
            <div>
              ...
              <FileUploaderWithPreview
                ...
                setImageFile={(file) => {
                  ...
                  if (errors.shortThumbnail)
                    setErrors((p) => ({ ...p, shortThumbnail: null }));
                }}
                name="shortThumbnail"
