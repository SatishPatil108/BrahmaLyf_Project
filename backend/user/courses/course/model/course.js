...
export const enrollInCourseModel = async (req, res) => {
  ...
  const response = await enrollInCourseService(userId, courseId, new Date(), 1);
  if (response == -1) {
    return error(
      res,
      HTTP_OK,
  ...
  // Initialize Tracking Questions state for this course (Week 1, Day 1)
  try {
    await runTrackingQuestionsCronForCourse(courseId);
    console.log(
      `[ENROLL] Initial tracking questions state created for Course ${courseId}`,
    );
  } catch (err) {
    console.error(
      `[ENROLL] Failed to initialize tracking questions for Course ${courseId}:`,
      err.message,
    );
  }

  // Initialize Tools Questions state for this course (Week 1, Day 1)
  try {
    await runToolsQuestionsCronForCourse(courseId);
    console.log(`[ENROLL] Tools questions initialized for Course ${courseId}`);
  } catch (err) {
    console.error(
      `[ENROLL] Failed to initialize tools questions for Course ${courseId}:`,
      err.message,
    );
  }
...
export const getCourseFeedbacksByCourseIdModel = async (req, res) => {
  ...
  const response = await getCourseFeedbacksByCourseIdService(
    courseId,
    pageNo,
    pageSize,
  );
...
export const searchCourseModel = async (req, res) => {
  const { pageNo, pageSize, courseName: cName, coachName: chName } = req.params;
